...
class InvoiceGenerationWorker {
  constructor() {
    this.queueUrl = QUEUE_URLS.INVOICE_GENERATION;
    this.dlqUrl = QUEUE_URLS.INVOICE_DLQ; // configure in AWS console
    this.isRunning = false;
    this.maxRetries = 3;
    this.visibilityTimeout = 30; // seconds
    console.log(`🔧 Invoice Generation Worker initialized with queue: ${this.queueUrl}`);
  }
  ...
  start() {
    ...
    this.isRunning = true;
    console.log(`🚀 Starting worker with long polling (10s)...`);
    this.poll();
  }
  ...
  async poll() {
    ...
    while (this.isRunning) {
      try {
        ...
        pollCount++;
        if (pollCount % 6 === 0) {
          console.log(`Worker heartbeat: ${pollCount} polls completed, still running...`);
        }

        if (response.Messages && response.Messages.length > 0) {
          console.log(`📨 Received ${response.Messages.length} messages`);
          await Promise.allSettled(response.Messages.map(msg => this.processMessage(msg)));
        }
      } catch (error) {
  ...
  async processMessage(message) {
    ...
    try {
      ...
      console.log(`🔄 Processing invoice generation for order: ${body.orderId}`);

      // extend visibility if needed
      const extendTimeout = setInterval(async () => {
        try {
          ...
          console.log(`⏳ Extended visibility timeout for order: ${body.orderId}`);
        } catch (err) {
          console.error(`⚠️ Failed to extend visibility timeout:`, err);
        }
      }, (this.visibilityTimeout - 5) * 1000); // extend 5s before expiry

      await this.processInvoiceGeneration(body);

      clearInterval(extendTimeout);
      await this.deleteMessage(message.ReceiptHandle);
      console.log(`✅ Processed invoice for order ${body.orderId}`);
    } catch (err) {
      console.error(`❌ Failed processing message:`, err);

      const retryCount = body?.retryCount || 0;
      if (retryCount < this.maxRetries) {
        console.log(`🔄 Retrying message (attempt ${retryCount + 1}/${this.maxRetries})`);
        // Requeue with incremented retry count
        await sqsClient.send(new SendMessageCommand({
          QueueUrl: this.queueUrl,
          MessageBody: JSON.stringify({ ...body, retryCount: retryCount + 1 })
        }));
      } else {
        console.error(`💀 Max retries exceeded. Sending message to DLQ`);
        await sqsClient.send(new SendMessageCommand({
  ...
  async processInvoiceGeneration(body) {
    const { orderId, hospitalId, courier, amount, invoiceDate, dueDate, status } = body;

    const existingBilling = await Billing.findOne({ order: orderId });
    if (existingBilling) {
      console.log(`⚠️ Billing already exists for order: ${orderId}`);
      return;
    }
    ...
    console.log(`📑 Invoice generated for order ${orderId}: Billing ID ${newBilling._id}, Amount: $${amount}`);
  }
